...
class DLBot(object):
    ...
    def __init__(self, token, user_id=None):
        ...
        self.startup_message = "Hi, I'm the DL bot! I will send you updates on your training process.\n" \
                               ...
                               " Send /pause to pause training.\n" \
                               " Send /resume to resume training.\n" \
    ...
    def activate_bot(self):
        ...
        dp.add_handler(self.pause_handler())  # pause training
        dp.add_handler(CommandHandler("resume", self.resume, filters=self.filters))
...
# pause training process callbacks
    def pause_training(self, bot, update):
        """ Telegram bot callback for the /stoptraining command. Displays verification message with buttons"""
        reply_keyboard = [['Yes', 'No']]
        update.message.reply_text(
                    'Are you sure? '
                    'This will pause your training process!\n\n',
                    reply_markup=ReplyKeyboardMarkup(reply_keyboard))
        return 1

    def pause_training_verify(self, bot, update):
        """ Telegram bot callback for the /pause command. Handle user selection as part of conversation"""
        is_sure = update.message.text  # Get response
        if is_sure == 'Yes':
            self.pause_train_flag = True
            open('pausefile','a').close()
            update.message.reply_text('OK, pausing training!', reply_markup=ReplyKeyboardRemove())
        elif is_sure == 'No':
            self.pause_train_flag = False  # to allow changing your mind before stop took place
            update.message.reply_text('OK, canceling pause request!', reply_markup=ReplyKeyboardRemove())

        return ConversationHandler.END

    def cancel_pause(self, bot, update):
        """ Telegram bot callback for the /pause command. Handle user cancellation as part of conversation"""
        self.pause_train_flag = False
        update.message.reply_text('OK, training will not be paused.',
                                  reply_markup=ReplyKeyboardRemove())
        return ConversationHandler.END

    def resume(self, bot, update):
        """ Telegram bot callback for the /resume command."""
        if self.pause_train_flag:
            self.pause_train_flag = False
            os.remove('pausefile')
            update.message.reply_text('Resuming training.',
                                    reply_markup=ReplyKeyboardRemove())
        return ConversationHandler.END

    def pause_handler(self):
        """ Function to setup the callbacks for the /pause command. Returns a conversation handler """
        conv_handler = ConversationHandler(
            entry_points=[CommandHandler('pause', self.pause_training, filters=self.filters)],
            states={1: [RegexHandler('^(Yes|No)$', self.pause_training_verify)]},
            fallbacks=[CommandHandler('cancel', self.cancel_pause, filters=self.filters)])
        return conv_handler
...
#        if bot is None and update is None and self.chat_id is not None:
#            self.updater.bot.send_photo(chat_id=self.chat_id, photo=buffer)
#        else:
    ...
    def plot_final_loss(self):
        ...
        if not self.loss_hist or plt or self.chat_id is None:
            # First epoch wasn't finished or matplotlib isn't installed
            return
        loss_np = np.asarray(self.loss_hist)
        # Check if training has a validation set
        ...
        fig.savefig(buffer, format='png')
        buffer.seek(0)        
        self.bbot.send_photo(self.chat_id, buffer)
