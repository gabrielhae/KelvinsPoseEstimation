...
class TelegramBotCallback(Callback):
    ...
    def __init__(self, kbot):
        ...
        self.kbot = kbot
        self.pause_file = 'pausefile'


    def on_train_begin(self, logs=None):
        logs = logs or {}
        logs['lr'] = K.get_value(self.model.optimizer.learning_rate)  # Add learning rate to logs dictionary
    ...
    def on_epoch_begin(self, epoch, logs=None):
        ...
        if self.kbot.modify_lr != 1:
            ...
            lr = float(K.get_value(self.model.optimizer.learning_rate))  # get current lr
            # new LR
    ...
    def on_batch_end(self,batch,logs=None):
        # Did user invoke PAUSE command
        if self.kbot.pause_train_flag and os.path.exists(self.pause_file):
            self.kbot.send_message('Training Paused!')
            print('\nTraining Paused! Pause command sent via Telegram bot.')
            ticker = threading.Event()
            while not ticker.wait(10) and os.path.exists(self.pause_file):
                pass #self.kbot.send_message('Training Paused!')