...
#    plt.show()    
    ...
    if Restart:
        ...
        model_final.load_weights('tAUGResnet50_300weights.97-0.26.hdf5')#''Resnet50_'+str(input_img_size[0])+'weights.77-1.26.hdf5')
...
#        model_final.load_weights("tmp.h5", by_name=True)        
    ...
    model_final.compile(loss={'q': geodesic_loss, 'r': mse},loss_weights=[1.0,1.0], optimizer=opt,metrics={'q': [geodesic_angular_loss], 'r': [r2_loss,ev_loss]})#,metrics=['accuracy','cosine_proximity','mean_squared_error',lr_metric])
...
####Main model training    str(input_img_size[0])+'_pose_bestmodel.hdf5'
    if Training:
        BestModel = ModelCheckpoint(filepath='tAUGResnet50_'+str(input_img_size[0])+'weights.{epoch:02d}-{val_loss:.2f}.hdf5', monitor='val_loss', verbose=1, save_best_only=True, save_weights_only=True, mode='min', period=1)
        csv_logger = CSVLogger(str(input_img_size[0])+'tAUGHistory_log.csv', append=True, separator=',')
