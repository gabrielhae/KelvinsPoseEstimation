...
from uncertainty_layer import LossWeighter,CurrentLossWeight
...
def main(speed_root, epochs, batch_size):
    ...
    opt = AdamAccumulate(lr=learning_rate, beta_1=0.9, beta_2=0.999,epsilon=None, decay=0., amsgrad=False, accum_iters=5)
...
#        model_final = Model(newInput, newOutputs)
        ...
        #x = keras.layers.GlobalAveragePooling2D(name='avg_pool')(pretrained_model.output)
...
#        rvecfork = keras.layers.Dropout(0.2)(rvecfork)
        ...
        x_vert = keras.layers.Reshape((32,64))(rvecfork)
        x_hori = keras.layers.Permute((2,1),input_shape=(32,64))(x_vert)
        
        yellow=keras.layers.LSTM(128,input_shape=(32,64),return_sequences=False,go_backwards=True,name='yellow')(x_vert)
        blue=keras.layers.LSTM(128,input_shape=(32,64),return_sequences=False,go_backwards=False,name='blue')(x_vert)
        orange=keras.layers.LSTM(128,input_shape=(64,32),return_sequences=False,go_backwards=True,name='orange')(x_hori)
        green=keras.layers.LSTM(128,input_shape=(64,32),return_sequences=False,go_backwards=False,name='green')(x_hori)
        
        rvecfork=keras.layers.concatenate([yellow,blue,orange,green],axis=-1)
        ...
        weightedLoss = LossWeighter(name='weighted_loss')([qLoss,rLoss])
        
        merged_output = keras.layers.concatenate([weightedLoss,qLoss,rLoss], axis=-1)
...
#            json_file.write(model_json)    
    ...
    #model_final.compile(optimizer=opt,loss=MainLoss,metrics=[QLoss,RLoss])#,metrics=['accuracy','cosine_proximity','mean_squared_error',lr_metric])
...
####Main model training
    if Training:#weights.{epoch:02d}-{val_loss:.2f}
        ...
        clr = CyclicLR(base_lr=1e-4,max_lr=4e-4,
...
#        # Training the model (transfer learning)
    ...
    model_final.load_weights(filepath='DenseNet121_LSTM300current_best.hdf5', by_name=True)#str(input_img_size[0])+'_pose_bestmodel.hdf5')
