...
#from keras_applications.resnet import ResNet34,preprocess_input
...
from HourglassPose import GetHourGlassPose
...
#                x = keras_image.array_to_img(image)
#                x = imgenhancer_Brightness = ImageEnhance.Brightness(x)
#                x = imgenhancer_Brightness.enhance(brightness)
#                image = keras_image.img_to_array(x)       
            
#            if bool(random.getrandbits(1)): #Randomly augment image to the distance
#                newz = r[2] + (random.random() * (40.0 - r[2]))
#                scale_downsize=r[2]/newz
#                image=self.clipped_zoom(image, scale_downsize)
#                r[2]=newz  
            ...
            if bool(random.getrandbits(1)):
                ...
                min_wh=200*self.width/1200
                max_wh=1000*self.width/1200
                
                new_x=min_wh + (random.random() * (max_wh - min_wh))
                new_y=min_wh + (random.random() * (max_wh - min_wh))
...
def main(speed_root, epochs, batch_size):
    ...
    opt = AdamAccumulate(lr=learning_rate, beta_1=0.9, beta_2=0.999,epsilon=None, 
                         decay=0., amsgrad=False, accum_iters=5)
    ...
    if Restart:
        ...
        json_file = open(file_prefix+str(input_img_size[0])+'model_final.json', 'r')
        ...
        model_final.load_weights('weightedtAUGResnet50_300weights.100-0.34.hdf5')#''Resnet50_'+str(input_img_size[0])+'weights.77-1.26.hdf5')
...
#        model_final = Model(newInput, newOutputs)
        ...
        with open(file_prefix+str(input_img_size[0])+'random_gen_state.obj', 'rb') as f:
        ...
        ####VGG16
        #pretrained_model = VGG16(weights="imagenet", include_top=False,input_shape=input_shape,pooling='avg')
        #x = pretrained_model.output

        #####Inceptionv4
        #pretrained_model = Inceptionv4(weights='imagenet',input_shape=input_shape, include_top=False,pooling='avg')
        #x = pretrained_model.output
        
        ####RESNEXT50
        #pretrained_model = ResNeXt50(weights="imagenet", include_top=False,input_shape=input_shape,pooling='avg',backend = K,layers = keras.layers, models = keras.models, utils = keras.utils)
        #x = pretrained_model.output
        
        ####Densenet121
        #pretrained_model = DenseNet121(weights="imagenet", include_top=False,input_shape=input_shape,pooling='avg')#,backend = K,layers = keras.layers, models = keras.models, utils = keras.utils)
        #x = pretrained_model.output
        
        ####InceptionV3
        #pretrained_model = InceptionV3(weights="imagenet", include_top=False,input_shape=input_shape,pooling='avg')#,backend = K,layers = keras.layers, models = keras.models, utils = keras.utils)
        #x = pretrained_model.output
        #x=keras.layers.Conv2D(filters=256, kernel_size=(3,3), strides=(2, 2),name='ConvBottleneck')(pretrained_model.output)
        #GAP=keras.layers.GlobalAveragePooling2D()(x)   
        #x=keras.layers.MaxPooling2D((2, 2))(x) 
        #x = keras.layers.concatenate([GAP,GMP], axis=-1)
        #x = keras.layers.Flatten()(x)
        ####Inception-Resnetv2
        #pretrained_model = InceptionResNetV2(weights="imagenet", include_top=False,input_shape=input_shape,pooling='avg')#,backend = K,layers = keras.layers, models = keras.models, utils = keras.utils)
        #x = pretrained_model.output             
        ...
        ####Resnet34 Stacked Hourglass
        #pretrained_model = ResNet34(input_shape=input_shape,classes=5)
#        repititions=[3,4,6,3]
#        pretrained_model = ResNet(input_shape=input_shape,
...
#        x = pretrained_model.output        
        ...
        ####HourglassPoseVarient
        pretrained_model = GetHourGlassPose(include_top=False,
                                         input_shape=input_shape,
                                         pooling=None)
        x = pretrained_model.output
#        x = Capsule(num_capsule=15,
#                             dim_capsule=16,
#                             routings=3,
#                             activation='relu',
#                             share_weights=True)(x)
        ...
        ####Attention56 # NEEDS PRETRAINING ON IMAGENET
        #pretrained_model = Attention56(include_top=False,input_shape=input_shape,pooling='avg')
        
        #StackedHourglass notworking
        #pretrained_model=StackedHourglass(10, num_stacks=1, num_channels=3,input_shape=input_shape)
        #x = pretrained_model.output
...
#        model_final = model_from_json(jsona,custom_objects={'LossWeighter': LossWeighter,'geodesic_loss':geodesic_loss,'mse':mse})
#        model_final.load_weights("tmp.h5", by_name=True)        
#        
#        #Pot the model
        plot_model(model_final, to_file=file_prefix+'model_final.png', show_shapes=True, show_layer_names=True)
#        
#        with open(file_prefix+str(input_img_size[0])+'random_gen_state.obj', 'wb') as f:
#            dump(np.random.get_state(), f)
#            
#        # serialize model to JSON
#        print("Saving model to disk")
#        model_json = model_final.to_json()
#        with open(file_prefix+str(input_img_size[0])+"model_final.json", "w") as json_file:
#            json_file.write(model_json)    
    
    model_final.compile(optimizer=opt,loss=MainLoss,metrics=[QLoss,RLoss,RTwoLoss])#,metrics=['accuracy','cosine_proximity','mean_squared_error',lr_metric])
...
#
####Main model training
    if Training:#weights.{epoch:02d}-{val_loss:.2f}
        #model_final.load_weights(filepath='Inceptionv3_with2ndrloss_production600current_best.hdf5', by_name=True)#str(input_img_size[0])+'_pose_bestmodel.hdf5')

        BestModel = ModelCheckpoint(filepath=file_prefix+str(input_img_size[0])+'current_best.hdf5', monitor='val_loss', verbose=1, save_best_only=True, save_weights_only=True, mode='min', period=1)
        csv_logger = CSVLogger(file_prefix+str(input_img_size[0])+'History_log.csv', append=True, separator=',')
        ...
        tboard_epoch=TensorBoard(log_dir='./logs/'+file_prefix+'/', 
                       histogram_freq=0, 
                       batch_size=32, 
                       write_graph=True, 
                       write_grads=False, 
        ...
        clr = CyclicLR(base_lr=1e-4,max_lr=1e-3,
                       step_size=1050,#1050,#1680,#1050, #change to 1050
                       mode='triangular2',
                       gamma=1.,
                       scale_fn=None,
                       scale_mode='cycle',
...
#        # Training the model (transfer learning)
        history = model_final.fit_generator(
                                ...
                                workers=4,
                                max_queue_size=40,
                                callbacks=[current_loss,clr,BestModel,csv_logger,early_stopping,tboard_epoch])
    ...
    #model_final.load_weights(filepath='Inceptionv3_300300current_best.hdf5', by_name=True)#str(input_img_size[0])+'_pose_bestmodel.hdf5')
...
#####Generating submission
    if submit:
        ...
        submission.export(suffix=file_prefix+str(input_img_size[0])+'results')      
...
model_final=main(dataset, int(150), int(16))