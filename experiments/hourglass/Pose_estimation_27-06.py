...
####Main model training    str(input_img_size[0])+'_pose_bestmodel.hdf5'
    if Training:
        BestModel = ModelCheckpoint(filepath='weightedtAUGResnet50_'+str(input_img_size[0])+'weights.{epoch:02d}-{val_loss:.2f}.hdf5', monitor='val_loss', verbose=1, save_best_only=True, save_weights_only=True, mode='min', period=1)
        csv_logger = CSVLogger(str(input_img_size[0])+'weightedtAUGHistory_log.csv', append=True, separator=',')
