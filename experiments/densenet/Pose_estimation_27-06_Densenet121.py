...
#        model_final = Model(newInput, newOutputs)
        ...
        ####Densenet121
        pretrained_model = DenseNet201(weights="imagenet", include_top=False,input_shape=input_shape,pooling='avg')#,backend = K,layers = keras.layers, models = keras.models, utils = keras.utils)
        x = pretrained_model.output
...
#        model_final.load_weights("tmp.h5", by_name=True)        
        ...
        plot_model(model_final, to_file='incepv4model_final.png', show_shapes=True, show_layer_names=True)
        ...
        with open(str(input_img_size[0])+"incepv4model_final.json", "w") as json_file:
...
####Main model training    str(input_img_size[0])+'_pose_bestmodel.hdf5'
    if Training:
        BestModel = ModelCheckpoint(filepath='weightedtAUGincepv4_'+str(input_img_size[0])+'weights.{epoch:02d}-{val_loss:.2f}.hdf5', monitor='val_loss', verbose=1, save_best_only=True, save_weights_only=True, mode='min', period=1)
        csv_logger = CSVLogger(str(input_img_size[0])+'weightedtAUGincepv4History_log.csv', append=True, separator=',')
