...
def evaluate(model, dataset, append_submission, dataset_root):
    ...
    Xtrans=np.array([-200,-150,-100,-50,0,50,100,150,200])
    Ytrans=Xtrans
    ...
    for img in tqdm(image_list):
        ...
        if tta:        
            ...
            #Randomly translate image to predict
            index=0 
            x_count=0
            y_count=0
            q_predictions = np.zeros((81, 4))
            rz_predictions = np.zeros((81, 1))
            ry_predictions = np.zeros((9, 1))
            rx_predictions = np.zeros((9, 1))
            for i in Xtrans:
                for j in Ytrans:
                    translate_x=Camera.nu-i
                    translate_y=Camera.nv-j
                    temp_image=x
                    temp_image=shift(temp_image,(translate_x,translate_y))
                    
                    tempx = preprocess_input(temp_image)
                    tempx = np.expand_dims(tempx, 0)
                    output = model.predict(tempx)
                    
                    q_predictions[index]=output[0]
                    rz_predictions[index]=output[1][0][2] 
                    index+=1    
                    if i==0:
                        rx_predictions[x_count]=output[1][0][0]
                        x_count+=1
                    if j==0:
                        ry_predictions[y_count]=output[1][0][1]
                        y_count+=1                    
                    
            average_q=averageQuaternions(q_predictions)
            mean_rx=np.mean(rx_predictions)
            mean_ry=np.mean(ry_predictions)
            mean_rz=np.mean(rz_predictions)
...
#            output[1][0][2]=mean_rz
            average_r=np.array([mean_rx,mean_ry,mean_rz])
            test=np.concatenate((average_q,average_r),axis=None)
...
class KerasDataGenerator(Sequence):#Sequence
        ...
        def __data_generation(self, list_IDs_temp):
            ...
            #return [X, y1,y2], dummy #training
            return X, [y1,y2] #prediction
...
#    plt.show()    
    ...
    if Restart:
        ...
        json_file = open(str(input_img_size[0])+'incepv4model_final.json', 'r')
...
#        model_final = Model(newInput, newOutputs)
        ...
        with open(str(input_img_size[0])+'incepv4random_gen_state.obj', 'rb') as f:
...
#        rvecfork = keras.layers.Dropout(0.5)(rvecfork)          
        ...
        q_true=Input((4,))#think this is reight decalre the input
        r_true=Input((3,))
        ...
        #model_final = Model([pred_model_final.input, q_true, r_true], weightedLoss)
...
#        plot_model(model_final, to_file='testmodel_final.png', show_shapes=True, show_layer_names=True)
#        
#        with open(str(input_img_size[0])+'testrandom_gen_state.obj', 'wb') as f:
#            dump(np.random.get_state(), f)
#            
#        # serialize model to JSON
#        print("Saving model to disk")
#        model_json = model_final.to_json()
#        with open(str(input_img_size[0])+"testmodel_final.json", "w") as json_file:
#            json_file.write(model_json)    
    
#    model_final.compile(optimizer=opt,loss=ignoreLoss)#,metrics=['accuracy','cosine_proximity','mean_squared_error',lr_metric])
    ...
    #[modified_geodesic_loss,'mean_squared_error']
...
####Main model training    str(input_img_size[0])+'_pose_bestmodel.hdf5'
    if Training:
        BestModel = ModelCheckpoint(filepath='test_'+str(input_img_size[0])+'weights.{epoch:02d}-{val_loss:.2f}.hdf5', monitor='val_loss', verbose=1, save_best_only=True, save_weights_only=True, mode='min', period=1)
        csv_logger = CSVLogger(str(input_img_size[0])+'testHistory_log.csv', append=True, separator=',')
...
#        # Training the model (transfer learning)
        ...
        with open(str(input_img_size[0])+'incepv4random_gen_state.obj', 'wb') as f:
            dump(np.random.get_state(), f)    
...
####
    model_final.load_weights('test_300weights.98--1.81.hdf5', by_name=True)
...
#####Generating submission
    if submit:
        ...
        submission.export(suffix=str(input_img_size[0])+'resultstta2')      
