...
class KerasDataGenerator(Sequence):#Sequence
        ...
        def __data_generation(self, list_IDs_temp):
            ...
            for i, ID in enumerate(list_IDs_temp):
                ...
                #pdb.set_trace()
                X[i,] = x 
                y1[i]=q#,y2[i] = q,r#np.concatenate([q, r])
                #y#1[i] = np.concatenate([q, r])
...
#        rvecfork = keras.layers.Dropout(0.5)(rvecfork)          
        ...
        qvec=keras.layers.Dense(4, activation="tanh",name='qtanh',kernel_regularizer=l2(0.01))(rvecfork)
        #qvec = Lambda(lambda  x: K.l2_normalize(x,axis=-1),name='q')(qvec)#,name="q_pred"
        
        #rvec=keras.layers.Dense(3, activation="linear",name='rlinear')(rvecfork)

        #out = keras.layers.concatenate([qvec, rvec])
        
        model_final = keras.models.Model(inputs=pretrained_model.input, outputs=qvec)
...
#            json_file.write(model_json)    
    ...
    model_final.compile(loss=[modified_geodesic_loss],loss_weights=[1.0], optimizer=opt,metrics=['mean_absolute_error'])#,metrics=[posvec_euclidean_distance_loss])#,metrics=['accuracy','cosine_proximity','mean_squared_error',lr_metric])
...
####Main model training    
    if Training:
        BestModel = ModelCheckpoint(filepath=str(input_img_size[0])+'_pose_bestmodel_qonly.hdf5', monitor='val_loss', verbose=1, save_best_only=True, save_weights_only=True, mode='min', period=1)
...
##        X2, Y2 = training_generator[45]
        losses = model_final.train_on_batch(X,Y)
        model_final.load_weights(filepath='restart_300_pose_bestmodel_qonly.hdf5')
...
#        print("losse")
        ...
        model_final.load_weights(filepath=str(input_img_size[0])+'_pose_bestmodel_qonly.hdf5')
...
model_final=main(dataset, int(50), int(16))
