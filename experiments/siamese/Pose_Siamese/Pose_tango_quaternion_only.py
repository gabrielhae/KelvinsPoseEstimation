...
class KerasDataGenerator(Sequence):#Sequence
        ...
        def __data_generation(self, list_IDs_temp):
            ...
            y1 = np.empty((self.batch_size, 7), dtype=np.float32)
...
#        rvecfork = keras.layers.Dropout(0.5)(rvecfork)          
        ...
        model_final = keras.models.Model(inputs=pretrained_model.input, outputs=out)
...
#            json_file.write(model_json)    
    ...
    model_final.compile(loss=[split_euclidean_distance_loss],loss_weights=[1.0], optimizer=opt,metrics=[posvec_euclidean_distance_loss])#,metrics=['accuracy','cosine_proximity','mean_squared_error',lr_metric])
...
model_final=main(dataset, int(20), int(16))
