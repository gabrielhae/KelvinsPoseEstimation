...
if selection==1:
    ...
    resolution='preprocessed_images_1200_cropped_resized_bbox'
else:
    resolution='images'
...
class KerasDataGenerator(Sequence):#Sequence
        ...
        def __init__(self, 
            ...
            self.q_poses=np.array([label['q_vbs2tango'] for label in label_list])
            self.n_channels = n_channels
            self.shuffle = shuffle
            self.augment = augment
            self.indexes_stream_a = None
            self.indexes_stream_b = None
            self.on_epoch_end()
            self.width = dim[0]
            self.height = dim[1]
        ...
        def __getitem__(self, index):
            ...
            indexes_stream_a = self.indexes_stream_a[index*self.batch_size:(index+1)*self.batch_size]

            # Find list of IDs
            list_IDs_temp_a = [self.list_IDs[k] for k in indexes_stream_a]

            # Generate data
            X, y = self.__data_generation(list_IDs_temp_a)
        ...
        def on_epoch_end(self):
            ...
            self.indexes_stream_a = np.arange(len(self.list_IDs))
            if self.shuffle:
                np.random.shuffle(self.indexes_stream_a)

        def __data_generation(self, list_IDs_temp_a):
            ...
            y2 = np.empty((self.batch_size, 4), dtype=np.float32)
            dummy = np.empty((self.batch_size, 1), dtype=np.float32)             
            ...
            ID_list=[]
            # Generate data
            for i, ID in enumerate(list_IDs_temp_a):
                ...
                q_list.append(q)#current q augmented accounted
                ID_list.append(ID)#filename
                #pdb.set_trace()
            ...
            for i, q in enumerate(q_list):
                ...
                #Y = distance.cdist(qmatch, q, 'euclidean')
                q=np.reshape(np.array(q), (1, 4))
                q_distances=distance.cdist(self.q_poses, q, 'euclidean')
                #q_distances=[self._numpy_geodesic_loss(qmatch, q) for qmatch in self.q_poses]
                #max_value = max(q_distances)
                max_index = np.argmax(q_distances)
                #min_value = min(q_distances)
                min_index = np.argmin(q_distances)               
                #sorted_qdistances = [[index,num] for index, num in sorted(enumerate(q_distances), key=lambda x: x[-1])]
                #shortest_imagename=self.labels[sorted_qdistances[0][0]]['filename']
                #farthest_imagename=self.labels[sorted_qdistances[-1][0]]['filename']
                if bool(random.getrandbits(1)):
                    #get closest pair ID
                    #ID_B=ID_list[i]
                    ID_B=self.list_IDs[min_index]
                else:
                    #get farthest pair ID
                    #ID_B=ID_list[i]
                    ID_B=self.list_IDs[max_index]
                        
                #filename=self.list_IDs[ID_B]
                #filename=
                #q_B = self.q_poses[ID_B]
                q_B = self.labels[ID_B]['q']
                img_path_B = os.path.join(self.image_root, ID_B)
                img_B = keras_image.load_img(img_path_B, target_size=self.dim)
                x_B = keras_image.img_to_array(img_B)                
                x_B = self.preprocessor(x_B)
                X2[i,] = x_B 
                y2[i]=q_B         
#
#                q, r = self.labels[ID]['q'], self.labels[ID]['r']
...
#                y2[i]=q
            ...
            y_concat=np.concatenate((y1, y2, dummy), axis=1)
            return [X1, X2], y_concat
        
        def _numpy_geodesic_loss(self,q1, q2):
...
def norm_penalty(y_pred, y_true):
...
def unit_norm_loss(y_pred, y_true):
    ...
    penalty = K.square(1.0 - norm)
    return K.mean(penalty, axis=0)

def euclidean_distance_loss(y_pred,y_true):
    y_pred=K.l2_normalize(y_pred,axis = 1)
    euclidean_distance=K.sqrt(K.sum(K.square(y_pred - y_true), axis=-1))
    return euclidean_distance

def split_euclidean_distance_loss(y_pred,y_true):
    
    q_y_pred=y_pred[:,:4]
    q_y_true=y_true[:,:4]
    q_y_pred=K.l2_normalize(q_y_pred,axis = 1)
    q_euclidean_distance=K.sqrt(K.sum(K.square(q_y_pred - q_y_true), axis=-1))
    
    r_y_pred=y_pred[:,4:]
    r_y_true=y_true[:,4:]    
    r_euclidean_distance=K.sqrt(K.sum(K.square(r_y_pred - r_y_true), axis=-1))
    
    return q_euclidean_distance+r_euclidean_distance


def posvec_euclidean_distance_loss(y_pred,y_true):
    
    r_y_pred=y_pred[:,4:]
    r_y_true=y_true[:,4:]    
    r_euclidean_distance=K.sqrt(K.sum(K.square(r_y_pred - r_y_true), axis=-1))
    
    return r_euclidean_distance

def geodesic_loss_with_norm_penalty(y_pred, y_true):
    q_y_pred=y_pred[:,:4]
    q_y_true=y_true[:,:4]    
    norm=K.sqrt(K.sum(q_y_pred*q_y_pred,axis=-1,keepdims=True))
    penalty = K.square(1.0 - norm)   
    
    q_y_pred=K.l2_normalize(q_y_pred,axis = -1)
    dotprod=K.sum(q_y_pred*q_y_true,axis=-1,keepdims=True)
    d = K.square(dotprod)

    loss=d + 10.0*penalty
    return K.maximum(loss,K.epsilon())

def geodesic_losscheck(y_pred, y_true):
    q_y_pred=y_pred[:,:4]
    q_y_true=y_true[:,:4]    
    #norm=K.sqrt(K.sum(q_y_pred*q_y_pred,axis=-1,keepdims=True))
    #penalty = K.square(1.0 - norm)   
...
#    loss=d #+ 10.0*penalty
    ...
    q0_minus_q1 = q_y_pred - q_y_true
    q0_plus_q1  = q_y_pred + q_y_true
    d_minus = K.l2_normalize(q0_minus_q1,axis = -1)
    d_plus  = K.l2_normalize(q0_plus_q1,axis = -1)
    if d_minus and d_plus is not None:
        if (d_minus < d_plus):
            return d_minus
        else:
            return d_plus    
...
def siam_loss(y_true,y_pred):
    """
    Combined Siamese Network Loss
    """
    A_y_pred = y_pred[:, 0:4]
    B_y_pred = y_pred[:, 4:8]
    pred_l2_distance = y_pred[:, 8]
    
    A_y_true = y_true[:, 0:4]
    B_y_true= y_true[:, 4:8]

    LossA=K.sqrt(K.sum(K.square(A_y_pred - A_y_true), axis=-1))
    LossB=K.sqrt(K.sum(K.square(B_y_pred - B_y_true), axis=-1))
    
    true_l2_distance=K.sqrt(K.sum(K.square(A_y_true - B_y_true), axis=-1))

    # distance between the anchor and the negative
    Lsiam=pred_l2_distance-true_l2_distance
 
    return Lsiam+LossA+LossB

def Qsiam_loss(y_true,y_pred):
    """
    Combined Siamese Network Loss
    """
    A_y_pred = y_pred[:, 0:4]
    A_y_pred=K.l2_normalize(A_y_pred,axis = -1)
    
    B_y_pred = y_pred[:, 4:8]
    B_y_pred=K.l2_normalize(B_y_pred,axis = -1)
    pred_q_distance = y_pred[:, 8]#okay
    
    A_y_true = y_true[:, 0:4]
    B_y_true= y_true[:, 4:8]
    
    #Compute Ground Truth Q Distance
    A_y_true=K.l2_normalize(A_y_true,axis = -1)
    B_y_true=K.l2_normalize(B_y_true,axis = -1)
    dotprod=K.sum(A_y_true*B_y_true,axis=-1,keepdims=True)
    true_q_distance = 1.0 - K.square(dotprod)

    # distance between the anchor and the negative
    Lsiam=pred_q_distance-true_q_distance
    
    #Loss A
    dotprod=K.sum(A_y_true*A_y_pred,axis=-1,keepdims=True)
    LossA = 1.0 - K.square(dotprod)    
    
    #Loss B
    dotprod=K.sum(B_y_true*B_y_pred,axis=-1,keepdims=True)
    LossB = 1.0 - K.square(dotprod)    
 
    return K.abs(Lsiam)+LossA+LossB

def lsiam(y_true,y_pred):
    """
    Siamese Network Loss Component
    """
    #Get Prediction L2 Difference computed via lambda Layer
    pred_l2_distance = y_pred[:, 8]
    
    A_y_true = y_true[:, 0:4]
    B_y_true= y_true[:, 4:8]
    
    #Compute Ground Truth L2 Distance
    true_l2_distance=K.sqrt(K.sum(K.square(A_y_true - B_y_true), axis=-1))

    # distance between the anchor and the negative
    Lsiam=true_l2_distance-pred_l2_distance
 
    return Lsiam

def Qlsiam(y_true,y_pred):
    """
    Siamese Network Loss Component
    """
    #Get Prediction q Difference computed via lambda Layer
    pred_q_distance = y_pred[:, 8]
...
def mymean_squared_error(y_true, y_pred):
...
#    print("--- %s seconds ---" % (time.time() - start_time))
    ...
    opt = AdamAccumulate(lr=learning_rate, beta_1=0.9, beta_2=0.999,
                 epsilon=None, decay=0., amsgrad=False, accum_iters=5)
...
#        model_final = Model(newInput, newOutputs)
        ...
        def q_distance(vects):
            x, y = vects            
            x=K.l2_normalize(x,axis = -1)
            y=K.l2_normalize(y,axis = -1)
            dotprod=K.sum(x*y,axis=-1,keepdims=True)
            result = 1.0 - K.square(dotprod)     
            return K.maximum(result, K.epsilon())
        
        def q_dist_output_shape(shapes):
        ...
        processed_a  = VGG16(weights="imagenet", include_top=False,input_tensor=input_a)
        
        
        for layer in processed_a.layers:
            layer.name = layer.name + str("_a")
        
        processed_b  = VGG16(weights="imagenet", include_top=False,input_tensor=input_b)
        
        for layer in processed_b.layers:
            layer.name = layer.name + str("_b")  
            
        x = processed_a.output
        x = keras.layers.GlobalAveragePooling2D(name='avg_pool_a')(x)
        x = keras.layers.Dense(2048, activation="relu")(x)
        qvec_a=keras.layers.Dense(4, activation="tanh",name='qtanh_a',kernel_regularizer=l2(0.01))(x)
        qvec_a = Lambda(lambda  x: K.l2_normalize(x,axis=-1),name='qa')(qvec_a)
        
        x = processed_b.output
        x = keras.layers.GlobalAveragePooling2D(name='avg_pool_b')(x)
        x = keras.layers.Dense(2048, activation="relu")(x)
        qvec_b=keras.layers.Dense(4, activation="tanh",name='qtanh_b',kernel_regularizer=l2(0.01))(x)      
        qvec_b = Lambda(lambda  x: K.l2_normalize(x,axis=-1),name='qb')(qvec_b)
        
        dcnn = Lambda(q_distance,output_shape=q_dist_output_shape)([qvec_a, qvec_b])
        
        merged_output = keras.layers.concatenate([qvec_a,qvec_b,dcnn], axis=-1)
        model_final = Model([input_a, input_b],output=merged_output)
#        model_final.save_weights("tmp.h5")
#        alpha = 0.02  # weight decay coefficient
#
#        for layer in model_final.layers:
...
#        pretrained_model.layers[-1].trainable = False   
        ...
        for layer in model_final.layers:
            print(layer, layer.trainable)            
#        model_final = model_from_json(model_final.to_json())
...
#            json_file.write(model_json)    
    ...
    model_final.compile(loss=Qsiam_loss, optimizer=opt,metrics=[Qlsiam,QLossA,QLossB])#,metrics=[posvec_euclidean_distance_loss])#,metrics=['accuracy','cosine_proximity','mean_squared_error',lr_metric])
    
    #model_final.load_weights(filepath='restart_300_pose_bestmodel_qonly.hdf5')
...
##    #lr_finder.find(x_train, y_train, start_lr=0.0001, end_lr=0.01, batch_size=batch_size, epochs=epochs)
        lr_finder.find_generator(training_generator, start_lr=1e-8, end_lr=1e-4, epochs=1, steps_per_epoch=None)
        lr_finder.plot_loss(n_skip_beginning=5, n_skip_end=1)
#
####Main model training    
    if Training:
        BestModel = ModelCheckpoint(filepath=str(input_img_size[0])+'sia_pose_bestmodel_qonly.hdf5', monitor='val_loss', verbose=1, save_best_only=True, save_weights_only=True, mode='min', period=1)
        csv_logger = CSVLogger(str(input_img_size[0])+'History_log_qonly.csv', append=True, separator=',')
    ...
    #                   update_freq='batch')        
        ...
        clr = CyclicLR(base_lr=0.5e-5,max_lr=1e-5,
                       step_size=1050,#1680,#1050, #change to 1050
                       mode='triangular2',
                       gamma=1.,
                       scale_fn=None,
                       scale_mode='cycle',
...
#        X1, X2, y = training_generator[0]#X1, X2, [y1, y2]
###        X2, Y2 = training_generator[45]
#        losses = model_final.train_on_batch([X1,X2],y)
#        model_final.load_weights(filepath='restart_300_pose_bestmodel_qonly.hdf5')
#        
##        predictions = model_final.predict_on_batch(X)
#        print(losses)
#        print("stopped")
        ...
        model_final.load_weights(filepath=str(input_img_size[0])+'sia_pose_bestmodel_qonly.hdf5')
...
####Predict on Validation Data  
    if predict:
        ...
        PlotinPLotly.send_to_plotly_q_results(train_labels,validation_labels,predictions)
...
model_final=main(dataset, int(100), int(16))
