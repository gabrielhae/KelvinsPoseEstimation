...
class KerasDataGenerator(Sequence):#Sequence
        ...
        def __getitem__(self, index):
            ...
            indexes_stream_b = self.indexes_stream_b[index*self.batch_size:(index+1)*self.batch_size]

            # Find list of IDs
            list_IDs_temp_a = [self.list_IDs[k] for k in indexes_stream_a]
            list_IDs_temp_b = [self.list_IDs[k] for k in indexes_stream_b]

            # Generate data
            X, y = self.__data_generation(list_IDs_temp_a,list_IDs_temp_b)
        ...
        def on_epoch_end(self):
            ...
            self.indexes_stream_b = np.arange(len(self.list_IDs))
            if self.shuffle:
                np.random.shuffle(self.indexes_stream_a)
                np.random.shuffle(self.indexes_stream_b)

        def __data_generation(self, list_IDs_temp_a,list_IDs_temp_b):
            ...
            for i, ID in enumerate(list_IDs_temp_b):
...
#    plt.show()    
    ...
    opt = AdamAccumulate(lr=learning_rate, beta_1=0.9, beta_2=0.999,
                 epsilon=None, decay=0., amsgrad=False, accum_iters=2)
...
#        model_final = Model(newInput, newOutputs)
        ...
        processed_a  = ResNet50(weights="imagenet", include_top=False,input_tensor=input_a)
        
        for layer in processed_a.layers:
            layer.name = layer.name + str("_a")
        
        processed_b  = ResNet50(weights="imagenet", include_top=False,input_tensor=input_b)
        ...
        dcnn = Lambda(euclidean_distance,output_shape=eucl_dist_output_shape)([qvec_a, qvec_b])
        
        merged_output = keras.layers.concatenate([qvec_a,qvec_b,dcnn], axis=-1)

        model_final = Model([input_a, input_b],output=merged_output)#[qvec_a,qvec_b,dcnn]
...
#            json_file.write(model_json)    
    ...
    model_final.compile(loss=siam_loss, optimizer=opt,metrics=[lsiam,LossA,LossB])#,metrics=[posvec_euclidean_distance_loss])#,metrics=['accuracy','cosine_proximity','mean_squared_error',lr_metric])
...
##    #lr_finder.find(x_train, y_train, start_lr=0.0001, end_lr=0.01, batch_size=batch_size, epochs=epochs)
        lr_finder.find_generator(training_generator, start_lr=1e-8, end_lr=1e-3, epochs=1, steps_per_epoch=None)
...
####Main model training    
    ...
    #                   update_freq='batch')        
        ...
        clr = CyclicLR(base_lr=0.5e-4,max_lr=1e-4,
