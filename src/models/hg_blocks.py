...
from keras.layers import Lambda,Input,Conv2D,BatchNormalization,SeparableConv2D,Add,MaxPool2D,UpSampling2D
...
#(num_classes=1001, dropout_prob=0.2, weights=None,input_tensor=None,input_shape=None, include_top=True,pooling='avg'):
def StackedHourglass(num_classes, num_stacks=2, num_channels=3, input_tensor=None,input_shape=None, bottleneck=bottleneck_block):
    #input = Input(shape=(inres[0], inres[1], 3))
    #input_shape=(int(300),int(300),int(3))
    ...
    front_features = create_front_module(inputs, num_channels, bottleneck)
