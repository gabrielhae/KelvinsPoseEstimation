...
#    plt.show()    
    ...
    if Restart:
        ...
        model_final.load_weights("Resnet50300weights.99-1.42.hdf5")
...
####Main model training    str(input_img_size[0])+'_pose_bestmodel.hdf5'
    if Training:
        BestModel = ModelCheckpoint(filepath='Resnet50'+str(input_img_size[0])+'weights.{epoch:02d}-{val_loss:.2f}.hdf5', monitor='val_loss', verbose=1, save_best_only=True, save_weights_only=True, mode='min', period=1)
