...
def main(speed_root, epochs, batch_size):
    ...
    opt = AdamAccumulate(lr=learning_rate, beta_1=0.9, beta_2=0.999,epsilon=None, decay=0., amsgrad=False, accum_iters=10)
...
####Main model training
    if Training:#weights.{epoch:02d}-{val_loss:.2f}
        ...
        BestModel = ModelCheckpoint(filepath=file_prefix+str(input_img_size[0])+'current_best_afterrestart.hdf5', monitor='val_loss', verbose=1, save_best_only=True, save_weights_only=True, mode='min', period=1)
...
#        # Training the model (transfer learning)
    ...
    #model_final.load_weights(filepath='Inceptionv3_with2ndrloss_production600current_best_afterrestart.hdf5', by_name=True)#str(input_img_size[0])+'_pose_bestmodel.hdf5')
